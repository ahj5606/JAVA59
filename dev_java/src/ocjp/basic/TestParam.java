 ...
 * TestParam.java를 컴파일 하면(고급언어->저급언어)
 * Param.class 와 TestParam.class가 만들어짐.
 * Param클래스에는 전변 ival이 있고 초기값은 현재 0임.
 * 그 값이 TestParam클래스의 main메소드에서 100으로 초기화
 * effectParam안에서는 500으로 초기화가 되었다.
 * 이 떄 main ival이 먼저 출력되는가? 아니면 sub ival이 먼저 출력되는가?
 * 둘 중에 먼저 출력된 값이 나중에 출력되는 곳에 영향을 미쳤다. 아니면 영향을 주지 않았다. 판단.
 * 두번째 변화 주기
 * 20번 주석이 있을대와 없을 때 결과에 차이가 있다|없다.
 * 만약 차이가 있다고 생각한다면 어떤 차이가 있는 것인지 옆사람에게 설명해 볼것.
 */
class Param{
	int ival = 0;
} //end of Param
